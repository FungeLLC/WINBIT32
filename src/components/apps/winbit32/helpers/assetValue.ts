...
import { warnOnce } from "@swapkit/helpers";
import { validateIdentifier } from "@swapkit/helpers";
import { BaseDecimal, Chain, type ChainId, ChainToChainId } from "@swapkit/helpers";
import type { TokenNames, TokenTax } from "@swapkit/helpers";
...
import type { SwapKitValueType } from "@swapkit/types";
...
export class AssetValue extends BigIntArithmetics {
  ...
  constructor({
    ...
    console.log("value in assetValue", value, decimal, tax, chain, symbol, identifier, decimalMultiplier);


    super(typeof value === "object" ? value : { decimal, value, decimalMultiplier: decimalMultiplier || 10n ** BigInt(decimal) });
...
or by passing asyncTokenLookup: true to the from() function, which will make it async and return a promise.`,
    ...
    const decimalMultiplier = fromBaseDecimal ? 10n ** BigInt(fromBaseDecimal) : 10n ** BigInt(decimal);
    console.log("decimalMultiplier in assetValue", decimalMultiplier, fromBaseDecimal, decimal, parsedValue, adjustedValue);
    ...
    const assetValue = asyncTokenLookup
      ...
      : isSynthetic || isTradeAsset
        ...
        : new AssetValue({ tax, decimal: fromBaseDecimal || decimal, identifier, value: adjustedValue, decimalMultiplier });
